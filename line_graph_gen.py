...
def generate_line_graph(num_vertices):
    """Generate an adjacency list for an undirected line graph."""
    if num_vertices < 2:
        raise ValueError("Number of vertices in a line graph must be at least 2.")
    
    graph_alist = [[] for _ in range(num_vertices)]
    for i in range(num_vertices):
        if i > 0:
            graph_alist[i].append(i - 1)
        if i < num_vertices - 1:
            graph_alist[i].append(i + 1)
    
    return graph_alist

def recur(adj_list, num_vertices, memo):
    ...
    for i in range(num_vertices):
        for j in range(num_vertices):
            if i != j and j not in adj_list[i] and i not in adj_list[j]:
                ...
                memo = recur(adj_list, num_vertices, memo)
                adj_list[i].remove(j)
                adj_list[j].remove(i)
...
num_vertices = 7
adj_list = generate_line_graph(num_vertices)
...
for i in combinations:
    graph_list = [list(k) for k in i]
    # print(graph_list)
    visited = brute(graph_list)
    for i in visited:
        if i == 0:
            print("NO")
            break