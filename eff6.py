...
def recur(state, dist_list, ecc, target_vertex):
    global recur_calls
    recur_calls += 1
    ...
    for i in range(len(weights)):
        ...
        if i != target_vertex and weights[i] > ecc:
            return tuple(visited)
    
    for i in range(len(weights)):
        if weights[i] > 0:
            for j in range(len(weights)):
                if weights[j] > 0 and dist_list[i][j] == weights[j] and j != target_vertex:
                    ...
                    result = recur((tuple(weights), tuple(visited)), dist_list, ecc, target_vertex)
...
def brute(graph_alist, target_vertex):
    """Apply brute force approach to solve the problem for a specific vertex."""
    weights = tuple([1] * len(graph_alist))
    dist_list = dist_list1(graph_alist)
    visited = tuple([0] * len(graph_alist))
    
    ecc = max(dist_list[target_vertex])
    print(f"Distance list: {dist_list}")
    print(f"Eccentricity for target vertex {target_vertex}: {ecc}")
    
    return recur((weights, visited), dist_list, ecc, target_vertex)
...
graph_alist = neighbors(3)
target_vertex = 4
print("Adjacency List:", graph_alist)
print("Final visited for target vertex {}: {}".format(target_vertex, brute(graph_alist, target_vertex)))
